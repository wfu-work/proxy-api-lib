...
// UsageStats describes Tokeni usage and balance details.
type UsageStats struct {
	Balance float64 `json:"balance"`
}

// UsageClient fetches Tokeni usage data.
...
func (c *UsageClient) Fetch(ctx context.Context, token string) (UsageStats, error) {
	...
	if token == "" {
		return UsageStats{}, errors.New("tokeni: token is required")
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return UsageStats{}, fmt.Errorf("tokeni: upstream returned %d", resp.StatusCode)
	}
	return ParseUsageResponse(body)
}

// ParseUsageResponse decodes Tokeni usage responses.
func ParseUsageResponse(body []byte) (UsageStats, error) {
	...
	if !ok {
		return UsageStats{}, errors.New("tokeni: invalid usage response")
	}
